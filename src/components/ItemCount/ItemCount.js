...
const ItemCount = ({stock, initial}) => {
    ...
    const addItem = () => {
        cant < stock ? setCant(cant + 1) : setWarning("no hay mas items disponibles en stock")
    }
    return (
        <div className="itemCount">
            <button className="itemCount__button" onClick= {removeItem} >-</button>
            <span className="itemCount__cant">{cant}</span>
            <button className="itemCount__button" onClick={addItem}>+</button>
